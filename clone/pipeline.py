...
class Pipeline:
    ...
    def run(self):
        ...
        input_file = "programs.pkl" if self.language == "c" else "bcb_funcs_all.tsv"
        if os.path.exists(os.path.join(self.root, self.language, "ast.pkl")):
            self.get_parsed_source(input_file="ast.pkl")
        else:
            self.get_parsed_source(input_file=input_file, output_file="ast.pkl")
        print("read id pairs...")
        if self.language == "c":
